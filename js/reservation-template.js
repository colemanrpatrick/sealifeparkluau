...
//___________________| Reservation UI V2.1|______________________
...
let securitySection = () => {
  ...
  securityHeaderImg.setAttribute(
    "src",
    "/designs/sealifeparkluau/images/design/shieldlock.svg"
  );
...
const getTime = (_timezone) => {
  //For HawaiiDiscount + microsites, use "Pacific/Honolulu" for timezone
  let datetime_str;

  if (_timezone == undefined) {
    datetime_str = new Date().toLocaleString("en-GB", { hour12: false });
  } else {
...
let disabledCalendarDates = ($x_) => {
  let disabledDates = $x_;
...
let showCalendar = (page, $collector) => {
  ...
  $("#datepicker").datepicker({
    ...
    beforeShowDay: function (date) {
      ...
      return [disabledCalendarDates(cartData.Availabilities[0].ClosedDates).indexOf(disabledDatesString) == -1];
    },
  });
  ...
  let dateArg = disabledCalendarDates(cartData.Availabilities[0].ClosedDates);
...
let returnCalendarData = () => {
  let calendarData = {
    ...
    currentTime: `${getTime()}`,
    hawaiiTime: `${getTime("Pacific/Honolulu")}`,
  };
  return calendarData;
};
//________________________________________________________
...
let showPrices = (page, dataPrices, priceGroup) => {
  ...
  Object.entries(dataPrices).forEach((entry) => {
    ...
    if (value.Grouping == priceGroup) {
      ...
      try {
        if (
          closedArgument.indexOf($selectedDate) == "-1" ||
          closedArgument == null
        ) {
